...
def build_ml_label_failure(settings, logger, batch) -> pd.DataFrame:
    ...
    base = base.sort_values(["well_id", "date"]).reset_index(drop=True)
    ...
    if not fail_path.exists():
        ...
        labels["failure_next_30d_flag"] = 0
    ...
    fail = fail.sort_values(["well_id", "fail_date"]).reset_index(drop=True)
    ...
    for well_id, g in base.groupby("well_id"):
        ...
        for _, row in g.iterrows():
            ...
            future_type = future.iloc[0]["failure_type"] if not future.empty and "failure_type" in future.columns else None
            ...
            out_rows.append(
                {
                    ...
                    "failure_within_14d": failure_within_14d,
                    "failure_within_30d": failure_within_30d,
                    "failure_next_30d_flag": int(failure_within_30d),
