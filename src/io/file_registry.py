...
def detect_incoming_files(settings, logger, batch):
    expected_files = [
        "production_data.csv",
        "master_well.csv",
        "chemical_rates.csv",
    ...
    for f in expected_files:
        ...
        path = RAW_DATA_DIR / f
        exists = path.exists()
        detected[f] = path.exists

        logger.info(f"Incoming file check:  {f} --> {'FOUND' if exists else 'MISSING'}")

    batch.incoming_files = detected    
